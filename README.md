...
- 💬 **Room chat** — side-by-side with the video on desktop, stacked below on mobile; as ephemeral as the room itself (history lives only while someone's in the room)
  ...
  - Typing shows as the message that's coming: the person's nametag over a bubble of animated dots, in the room chat and the fullscreen chat alike — and it's gone within a second of them stopping
  - A soft ping for every message that isn't yours, wherever you are (mutable via the 🔔 toggle). Away in another tab? The tab title also shows an unread count and the favicon gets a red dot
  - Spam control: more than five messages in five seconds and the extras are dropped, with a "slow down" note to the sender only — the room never sees the flood
...
- 🧭 **Recommendations that play in the room** — a "Recommended" list beside the video (left of it on a wide screen, under the chat on a narrower one), the same shape as the queue: play one now, or **+** it onto the end. Same source as YouTube's own (its mix for whatever is playing), already filtered to videos that really play embedded; YouTube only builds mixes for music, so for anything else Sesh asks for what YouTube's own watch page lists beside the video — its real "Up next" column, which exists for every video and costs no API quota. When YouTube won't answer at all — it rate-limits the address a server calls from — the list falls back to the rest of that channel's videos through the official API, and says so: the heading reads **More from this channel** rather than Recommended, because they aren't the same promise. Autoplay draws from the same well, so it keeps a room going after a documentary just as it does after a song. It exists because the suggestions *inside* the player can't work — the embed is sandboxed, so clicking one of YouTube's does nothing, and no page can see a click inside someone else's iframe, let alone redirect it
- ⏭️ **The handoff between videos** — when autoplay has picked something from YouTube's "Up next" (anything that isn't music), the end screen names it and counts down five seconds, with **Play now** and **Cancel** — the room's countdown, so whoever presses either does it for everyone, and a tab that joins mid-count sees the same clock. Music is the exception, on purpose: a Mix is a continuous run of songs, and YouTube plays those straight through, so Sesh does too
- 🎛️ **Our own player controls** — the embed runs with YouTube's chrome switched off and Sesh draws the bar: play/pause, a scrubber with buffered progress, volume, time, captions and fullscreen. The reason is measured, not cosmetic — YouTube's own controls hide about 3.6 seconds after the last mouse movement *they* see, and an embed never learns the pointer has left the player (blurring the iframe, `pointer-events: none`, synthetic `mouseout` and flashing its visibility all leave the same 3.6s). Ours goes the instant the pointer does, taking the mouse cursor with it, and carries the title, play/pause, a scrubber, volume, time, autoplay, captions, quality and fullscreen. What `controls=0` doesn't switch off — the title bar, and the share / Watch Later / "More videos" strip YouTube draws whenever the video is paused — is clipped out of sight instead: the iframe is taller than the picture and sits pulled up inside a window that crops it, so YouTube's chrome lands in the cropped-off strips while the video itself is untouched. Playback speed is deliberately not offered: the room is synchronized against the clock, so one person at 1.5× isn't watching the same thing as anyone else
...
- 🕘 **History** — a tab beside "Up next" lists what the room has played this sitting, newest first; **▶** plays one again for everyone
- ⌨️ **Keyboard shortcuts** — the few worth remembering, no cheat sheet needed: `Space` play/pause, `←` `→` five seconds back or ahead, `F` fullscreen, `C` chat (focuses the box, or toggles the panel in fullscreen), `M` mute. They work the same whether you last clicked the video or the page around it, and stay out of the way while you're typing (`Esc` leaves the chat box)
- 🖥️ **Fullscreen with chat** — Sesh's own fullscreen button, in the "Up next" header where it can't clash with YouTube's own controls and costs no space of its own (and leaving fullscreen puts the page back where you were, not at the top), and a chat panel you can toggle over the video: same conversation as the room chat, plus an unread badge while it's closed. No more choosing between the video filling the screen and knowing what everyone's saying
...
Sign in with `sesh login` (`sesh whoami`, `sesh logout`) and the terminal gets the account features too: `/friends` shows who's online and who's in a room, `/invite <n>` pulls one here, `/join <n>` and `/accept` move you into their room without restarting, and `/dms` / `/dm <n|name>` are direct messages, with `/room` (or Esc) going back to room chat. `/autoplay [on|off]` controls the room's radio (`/next` starts its pick now, `/cancel` calls off the countdown), and `/bug <description>` files a report without leaving the terminal. Without signing in the CLI behaves exactly as it always has.
...
Everything else — playback sync, chat, the queue, history, presence, autoplay — travels over the Socket.IO connection, not URLs.
