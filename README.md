...
- 🛡️ **No accidental exits** — the video body belongs to the room: clicking it toggles play/pause, like any player, instead of falling through to YouTube's title bar, paused-screen suggestions or creators' end-screen links, all of which open youtube.com over your sesh. What YouTube draws for *you* stays YouTube's and stays usable — the scrubber, the volume button *and its slider*, captions and quality in the top-right corner, and the settings menu that opens under them — while the row below the scrubber, which is nothing but "More videos" and "Watch on YouTube", doesn't. Use one of YouTube's own controls and the shield stands aside for a few seconds, so anything that opens can be operated normally. Nothing of ours is drawn over the picture either: what's coming up lives in the panel under the player, where it doesn't compete with the film
