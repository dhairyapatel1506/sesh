...
- 🧭 **Recommendations that play in the room** — a "Recommended" list beside the video (left of it on a wide screen, under the chat on a narrower one), the same shape as the queue: play one now, or **+** it onto the end. Same source as YouTube's own (its mix for whatever is playing), already filtered to videos that really play embedded; YouTube only builds mixes for music, so anything else falls back to a search for the video's title (pricier against the API quota, cached for a day). It exists because the suggestions *inside* the player can't work — the embed is sandboxed, so clicking one of YouTube's does nothing, and no page can see a click inside someone else's iframe, let alone redirect it
